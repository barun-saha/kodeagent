...
def test_add_to_history(react_agent):
    ...
    content = react_agent.chat_history[0]['content']
...
def test_format_messages_for_prompt(react_agent):
    ...
    formatted = react_agent.chat_history
...
async def test_agent_with_no_tools():
    ...
    with patch('kodeagent.kutils.call_llm', side_effect=llm_side_effect):
        agent = ReActAgent(name='no_tool_agent', model_name=MODEL_NAME, tools=[], max_iterations=1)
...
def test_chat_history_structure_react():
    """Test chat_history structure for ReActAgent."""
    agent = ReActAgent(name='test_agent', model_name=MODEL_NAME, tools=[calculator])
...
def test_chat_history_structure_codeact():
    """Test chat_history structure for CodeActAgent."""
...
def test_formatted_history_with_final_answer():
    ...
    formatted = agent.chat_history
    assert len(formatted) > 0
    # Should have assistant message with content (final_answer)
    assert any(
        m.get('role') == 'assistant' and m.get('content') == 'The answer is 42' for m in formatted
    )
...
def test_codeact_formatted_history_with_final_answer(codeact_agent_factory):
    ...
    # Should have assistant message with content (final_answer)
    assert any(
        m.get('role') == 'assistant' and m.get('content') == 'The result is 42' for m in formatted
    )
...
async def test_run_with_invalid_files_type(react_agent):
    ...
    with pytest.raises(ValueError, match='Task files must be a list of file paths!'):
        # Iterate over the async generator to trigger execution
        async for _ in react_agent.run(task='Do something', files='invalid_file_arg'):
            pass
...
async def test_record_thought_adds_missing_role(react_agent):
    ...
    with patch.object(react_agent, '_chat', return_value=response_no_role):
        # Call _record_thought using ReActChatMessage schema
        msg = await react_agent._record_thought(ReActChatMessage)

        assert msg is not None
        assert isinstance(msg, ReActChatMessage)
...
async def test_record_thought_parsing_failures(mock_agent):
    ...
    feedback_msgs = []
    for m in mock_agent.chat_history:
        content = m['content']
        text = ''
        if isinstance(content, list):
