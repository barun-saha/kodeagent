...
async def test_run_code_e2b_execution_error(mock_security_reviewer):
    ...
    with patch('e2b_code_interpreter.Sandbox', mock_e2b.Sandbox):
        ...
        _stdout, _stderr, _exit_status, _ = await runner.run('', "print('test')", task_id='1')
...
def test_code_runner_pip_parsing_logic():
    ...
    runner = CodeRunner(
        env='host', allowed_imports=[], pip_packages='numpy,pandas', model_name='test-model'
    )
    ...
    runner = CodeRunner(
        env='host',
        allowed_imports=[],
        pip_packages='numpy, pandas; requests ',
        model_name='test-model',
    )
    assert runner.pip_packages == ['numpy', ' pandas', ' requests ']

    # 4. Test empty/None
    runner_none = CodeRunner(
        env='host', allowed_imports=[], pip_packages=None, model_name='test-model'
    )
...
async def test_run_code_e2b_pip_install_runs_with_packages(mock_security_reviewer):
    ...
    with patch.dict(sys.modules, {'e2b_code_interpreter': mock_e2b}):
        runner = CodeRunner(
            env='e2b', allowed_imports=[], pip_packages='uvicorn', model_name='test-model'
        )
...
def test_code_runner_with_empty_pip_packages():
    ...
    runner = CodeRunner(
        env='host', allowed_imports=['os'], pip_packages='', timeout=30, model_name='test-model'
    )
...
async def test_code_runner_disallowed_imports_error(mock_security_reviewer):
    ...
    stdout, stderr, exit_code, _ = await runner.run(
        tools_code='', generated_code=code, task_id='task-1234'
    )
...
async def test_security_review_blocks_unsafe_code():
    ...
    with patch('kodeagent.code_runner.CodeSecurityReviewer') as mock_reviewer_class:
        ...
        assert (
            'pattern detection' in str(excinfo.value).lower()
            or 'security concerns' in str(excinfo.value).lower()
        )
...
async def test_security_review_allows_safe_code(mock_security_reviewer):
    ...
    with patch('subprocess.run') as mock_sp_run:
        ...
        with patch('os.remove'):
            stdout, stderr, exit_code, _ = await runner.run(
                tools_code='', generated_code=safe_code, task_id='test-2'
            )
...
async def test_security_review_called_with_generated_code_only():
    ...
    with patch('kodeagent.code_runner.CodeSecurityReviewer') as mock_reviewer_class:
        ...
        with patch('subprocess.run') as mock_sp_run:
            ...
            with patch('os.remove'):
                await runner.run(
                    tools_code=tools_code, generated_code=generated_code, task_id='test-3'
                )
            ...
            with patch('os.listdir', return_value=[]):
                ...
                with patch('kodeagent.code_runner.open', m, create=True):
                    ...
                    with patch('shutil.copy2'):
                        await runner.run(
                            tools_code=tools_code, generated_code=generated_code, task_id='test-4'
                        )
...
def test_code_runner_init_with_model_params():
    ...
    with patch('kodeagent.code_runner.CodeSecurityReviewer') as mock_reviewer_class:
        ...
        runner = CodeRunner(
            env='host', allowed_imports=['os'], model_name=model_name, litellm_params=litellm_params
        )
...
def test_code_runner_init_with_usage_tracker():
    ...
    with patch('kodeagent.code_runner.CodeSecurityReviewer') as mock_reviewer_class:
        ...
        _runner = CodeRunner(
            env='host', allowed_imports=['os'], model_name='test-model', usage_tracker=usage_tracker
        )
...
def test_code_runner_init_with_tool_names():
    ...
    with patch('kodeagent.code_runner.CodeSecurityReviewer') as mock_reviewer_class:
        ...
        runner = CodeRunner(
            env='host', allowed_imports=['os'], model_name='test-model', tool_names=tool_names
        )

        mock_reviewer_class.assert_called_once_with(
            model_name='test-model', litellm_params=None, usage_tracker=None, tool_names=tool_names
        )
        assert runner.code_reviewer == mock_instance
