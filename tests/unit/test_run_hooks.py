...
from kodeagent.kodeagent import AgentResponse, ReActAgent
...
class TestAgent(ReActAgent):
    ...
    async def post_run(self) -> AsyncIterator[AgentResponse]:
        """Set flag and call parent post_run."""
...
async def test_hooks_execution():
    ...
    agent._think.return_value = iter(
        []
    )  # async iterator mock is tricky, let's just mock run dependencies
