...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('junk {"key": "value"} more junk', '{"key": "value"}'),
        (' {"key": "value"}\n\t', '{"key": "value"}'),
    ],
)
...
class LLMDummyResponse:
    ...
    class Choices:
        """Mock for response choices."""

        class Message:
            """Mock for response message."""
...
async def test_call_llm_with_response_format(monkeypatch):
    ...
    result = await call_llm(
        'model', {}, [{'role': 'user', 'content': 'hi'}], response_format=DummyPydanticModel
    )
    assert result == '{"name": "test"}'
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('https://example.com/doc', {'Content-Type': 'application/pdf'}, 'application/pdf'),
        ('https://example.com/unknown', {}, 'Unknown file type'),
    ],
)
...
def test_make_user_message_with_urls(mock_head, mock_get):
    ...
    message = make_user_message(
        'Check these URLs',
        files=['https://example.com/image.jpg', 'https://example.com/document.pdf'],
    )
...
def test_make_user_message_with_images(mock_mime, mock_isfile):
    ...
    with patch('builtins.open', m):
        message = make_user_message('Check this image', files=['photo.jpg'])
...
def test_make_user_message_local_image_no_mime_type(mock_mime, mock_isfile):
    ...
    with patch('builtins.open', new_callable=MagicMock) as mock_file_open:
        ...
        message = make_user_message('Check this image', files=['photo.png'])
