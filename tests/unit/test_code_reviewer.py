...
async def test_code_reviewer_review_safe_code():
    ...
    with patch('kodeagent.kutils.call_llm') as mock_llm:
        ...
        reviewer = CodeSecurityReviewer(model_name='test-model', litellm_params={})
...
async def test_code_reviewer_with_litellm_params():
    ...
    with patch('kodeagent.kutils.call_llm') as mock_llm:
        ...
        reviewer = CodeSecurityReviewer(model_name='test-model', litellm_params=litellm_params)
...
async def test_code_reviewer_model_name_passed():
    ...
    with patch('kodeagent.kutils.call_llm') as mock_llm:
        ...
        reviewer = CodeSecurityReviewer(model_name=model_name, litellm_params={})
...
async def test_code_reviewer_with_whitelisted_calculator_tool():
    ...
    mock_response = (
        '{"is_secure": true, "reason": "Code uses whitelisted calculator tool which is safe"}'
    )
    ...
    with patch('kodeagent.kutils.call_llm') as mock_llm:
        ...
        reviewer = CodeSecurityReviewer(
            model_name='test-model', litellm_params={}, tool_names={'calculator'}
        )
...
async def test_code_reviewer_with_multiple_whitelisted_tools():
    ...
    with patch('kodeagent.kutils.call_llm') as mock_llm:
        ...
        reviewer = CodeSecurityReviewer(
            model_name='test-model', litellm_params={}, tool_names=tool_names
        )
